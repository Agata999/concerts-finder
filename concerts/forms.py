...
class SearchBandConcertsForm(forms.Form):
    ...
    city = forms.ModelChoiceField(
        label="Jeśli chcesz, wybierz miasto",
        widget=forms.Select,
        queryset=ConcertCity.objects.all().order_by("name"),
        required=False,
    )
    start_date = forms.DateField(
        label="Jeśli chcesz, wskaż najwcześniejszy termin koncertu",
        widget=forms.SelectDateWidget,
        required=False,
    )
    end_date = forms.DateField(
        label="Jeśli chcesz, wskaż najpóźniejszy termin koncertu",
        widget=forms.SelectDateWidget,
        required=False,
    )
...
class SearchArtistConcertsForm(forms.Form):
    last_name = forms.CharField(
        label="Podaj nazwisko artysty", max_length=64, required=False
    )
    artistic_name = forms.CharField(
        label="Lub pseudonim", max_length=64, required=False
    )
    city = forms.ModelChoiceField(
        label="Jeśli chcesz, wybierz miasto",
        widget=forms.Select,
...
class SearchDateConcertsForm(forms.Form):
    ...
    city = forms.ModelChoiceField(
        label="Jeśli chcesz, wybierz jeszcze miasto",
        widget=forms.Select,
        queryset=ConcertCity.objects.all().order_by("name"),
        required=False,
    )
...
class RegisterUserForm(forms.Form):
    ...
    conf_password = forms.CharField(
        label="Potwierdź hasło:", widget=forms.PasswordInput
    )
